...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      await connectDB();
      // Find leave requests where the managerEmail matches the current admin's email
  ...
  if (req.method === "PUT") {
    try {
        ...
        const updatedLeave = await Leave.findOneAndUpdate(
            { _id: leaveId, managerEmail: session.user.email }, // Ensure admin can only update their assigned requests
            { status },
            { new: true }
        );
        ...
        console.error("Error updating leave status for admin:", error);
