...
const Sidebar = ({ isPinned, setIsPinned }) => {
  ...
  if (isAdmin) {
    navLinks = [
      ...
      },
      {
        ...
        icon: FiBriefcase,
        subLinks: [
          { name: "Company Info", href: "/admin/company-info" },
    ...
    navLinks = [
      ...
      { name: "Assigned Users", href: "/manager/user-assigned", icon: FiUsers },
      {
        ...
        subLinks: [
          ...
          { name: "My History", href: "/manager/my-leaves" }, // Assuming manager has their own history page
  ...
  if (isAdmin || isManager) {
    resignationSubLinks.push({
      name: "Manage Requests",
      href: "/resignation/manage",
      roles: ["admin", "manager"],
      count: counts.resignations,
    });
  }
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-700 h-16">
          <h1
            className={`font-bold text-xl overflow-hidden whitespace-nowrap transition-all ${
                  ...
                  <div className="flex items-center">
                    <div
                      className={`transition-all duration-300 ${
                        isOpen ? "w-5" : "w-10 flex justify-center"
                      }`}
                    >
                      <link.icon size={20} />
                    </div>
                    <span
                      className={`ml-4 whitespace-nowrap overflow-hidden transition-all duration-200 ${
                ...
                {isOpen && openDropdown === link.name && (
                  <div className="pl-8 mt-1 space-y-1">
                    {link.subLinks.map((subLink) => {
                      if (
                        subLink.roles &&
                        !subLink.roles.includes(session?.user?.role)
                      )
                        return null;
                      const totalSubLinkCount = link.subLinks.reduce(
                        (acc, sl) => acc + (sl.count || 0),
                        0
                      );
                      return (
                        <Link
